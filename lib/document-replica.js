...
class DocumentReplica {
  ...
  updateUndoCount (opId, newUndoCount) {
    ...
    // Later nodes are higher in the tree due to earlier splays to get
    // segment positions, so we iterate in reverse when splaying a second
    // time to update nodes to account for the impact of the undo.
  ...
  verifyTreeInvariants () {
    this.verifySegmentsInTree()
    this.verifyPositionsInTree()
  }

  verifySegmentsInTree () {
    const treeSegments = this.getSegmentsInTree()
    const listSegments = this.getSegments()
    assert.deepEqual(treeSegments, listSegments)
  }

  verifyPositionsInTree () {
    let position = 0
    let segment = this.firstSegment
    while (segment) {
      if (this.isSegmentVisible(segment)) {
        assert.equal(this.getSegmentPosition(segment), position)
