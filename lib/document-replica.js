...
class DocumentReplica {
  constructor (siteId) {
    ...
    this.firstSegment = {opId: {site: 0, seq: 0}, offset: 0, pos: 0, text: '', nextSplit: null, deletions: new Set()}
  ...
  deleteLocal ({position, extent}) {
    ...
    while (true) {
      ...
      segment = getDocumentSuccessor(segment)
    }
  ...
  insertRemote ({opId, text, leftDependencyId, offsetInLeftDependency, rightDependencyId, offsetInRightDependency}) {
    ...
    let currentSegment = getDocumentSuccessor(originalLeftDependency)
    ...
    while (currentSegment !== rightDependency) {
      if (currentSegment.leftDependency.pos <= leftDependency.pos && currentSegment.rightDependency.pos >= rightDependency.pos) {
        ...
        currentSegment = getDocumentSuccessor(leftDependency)
      } else {
        currentSegment = getDocumentSuccessor(currentSegment)
      }
    }
    ...
    while (segment) {
      ...
      if (position <= segmentStart && segment !== this.firstSegment) {
        ...
        if (position < segmentEnd) {
          ...
          return [segment, getDocumentSuccessor(segment)]
        }
      }
    }
  ...
  splitSegment (segment, offset) {
    ...
    suffix.pos = (segment.pos + getDocumentSuccessor(segment).pos) / 2
...
function getDocumentSuccessor (node) {
  if (node.documentRight) {
    node = node.documentRight
    while (node.documentLeft) {
      node = node.documentLeft
    }
  } else {
    while (node.documentParent && node.documentParent.documentRight === node) {
