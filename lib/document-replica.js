...
class DocumentReplica {
  ...
  setTextInRange (start, end, text) {
    ...
    const operation = {type: 'splice', opId}

    if (compare(end, start) > 0) {
      operation.deletion = this.delete(opId, start, end)
    }
    if (text && text.length > 0) {
      operation.insertion = this.insert(opId, start, text)
    }
  ...
  canIntegrateOperation (op) {
    switch (op.type) {
      case 'splice': {
        const {opId, deletion, insertion} = op

        if ((this.maxSeqsBySite[opId.site] || 0) !== opId.seq - 1) {
          return false
        }
    ...
    while (i < operations.length) {
      ...
      if (this.canIntegrateOperation(operation)) {
        ...
        switch (operation.type) {
          case 'splice':
            if (operation.deletion) this.deleteRemote(operation.opId, operation.deletion)
            if (operation.insertion) this.insertRemote(operation.opId, operation.insertion)
            this.updateMaxSeqsBySite(operation.opId)
            break
          case 'undo':
  ...
  collectDeferredOperations ({opId}, operations) {
    if (opId) {
      const opIdString = opIdToString(opId)
  ...
  deferOperation (op) {
    if (op.type === 'splice') {
      const {opId, deletion, insertion} = op
      this.addOperationDependency(this.deferredOperationsByDependencyId, {site: opId.site, seq: opId.seq - 1}, op)
  ...
  textUpdatesForOperations (operations, oldUndoCounts) {
    ...
    for (let i = 0; i < operations.length; i++) {
      const operation = operations[i]
      const {type, opId, deletion, insertion} = operation
      if (opId) {
        const opIdString = opIdToString(opId)
  ...
  collectSegments (opIdString, segments, segmentIndices, segmentStartPositions) {
    ...
    const deletion = this.deletionsByOpId.get(opIdString)
    if (deletion) {
      const {
        leftDependencyId, offsetInLeftDependency,
        rightDependencyId, offsetInRightDependency,
