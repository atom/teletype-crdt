...
class DocumentReplica {
  ...
  setTextInRange (start, end, text) {
    const operation = {type: 'splice'}

    if (compare(end, start) > 0) {
      operation.deletion = this.delete(start, end)
    }

    if (text && text.length > 0) {
      operation.insertion = this.insert(start, text)
    }
  ...
  undoOrRedoOperations (operationsToUndo) {
    ...
    for (var i = 0; i < operationsToUndo.length; i++) {
      const {deletion, insertion} = operationsToUndo[i]
      const operation = {type: 'undo'}

      if (deletion) {
        const opId = deletion.opId
        const newUndoCount = (this.undoCountsByOpId.get(opIdToString(opId)) || 0) + 1
        this.updateUndoCount(opId, newUndoCount, oldUndoCounts)

        operation.deletion = {opId, undoCount: newUndoCount}
      }

      if (insertion) {
        const opId = insertion.opId
        const newUndoCount = (this.undoCountsByOpId.get(opIdToString(opId)) || 0) + 1
        this.updateUndoCount(opId, newUndoCount, oldUndoCounts)

        operation.insertion = {opId, undoCount: newUndoCount}
      }
  ...
  canIntegrateOperation (op) {
    switch (op.type) {
      case 'splice': {
        const {deletion, insertion} = op

        if (deletion) {
          const hasLeftAndRightDependencies = (
            this.splitTreesByOpId.has(opIdToString(deletion.leftDependencyId)) &&
            this.splitTreesByOpId.has(opIdToString(deletion.rightDependencyId)) &&
            (this.maxSeqsBySite[deletion.opId.site] || 0) === deletion.opId.seq - 1
          )
        ...
        if (insertion) {
          const hasLeftAndRightDependencies = (
            this.splitTreesByOpId.has(opIdToString(insertion.leftDependencyId)) &&
            this.splitTreesByOpId.has(opIdToString(insertion.rightDependencyId))
          )

          if (deletion) {
            return (
              hasLeftAndRightDependencies &&
              deletion.opId.seq === insertion.opId.seq - 1
            )
          } else {
            return (
              hasLeftAndRightDependencies &&
              (this.maxSeqsBySite[insertion.opId.site] || 0) === insertion.opId.seq - 1
            )
          }
        }
      ...
      case 'undo': {
        const {deletion, insertion} = op
        return (
          (!deletion || this.deletionsByOpId.has(opIdToString(deletion.opId))) &&
          (!insertion || this.splitTreesByOpId.has(opIdToString(insertion.opId)))
        )
      }
    ...
    while (i < operations.length) {
      ...
      if (this.canIntegrateOperation(operation)) {
        ...
        switch (operation.type) {
          case 'splice':
            if (operation.deletion) this.deleteRemote(operation.deletion)
            if (operation.insertion) this.insertRemote(operation.insertion)
            break
          case 'undo':
            if (!oldUndoCounts) oldUndoCounts = new Map()

            if (operation.deletion) this.undoRemote(operation.deletion, oldUndoCounts)
            if (operation.insertion) this.undoRemote(operation.insertion, oldUndoCounts)
            break
        }
  ...
  collectDeferredOperations (operation, operations) {
    if (operation.type === 'markerUpdates') return

    const {deletion, insertion} = operation

    if (deletion) {
      const opIdString = opIdToString(deletion.opId)
    ...
    if (insertion) {
      const opIdString = opIdToString(insertion.opId)
      const dependentOps = this.deferredOperationsByDependencyId.get(opIdString)
      if (dependentOps) {
        dependentOps.forEach((dependentOp) => {
          if (this.canIntegrateOperation(dependentOp)) {
  ...
  deferOperation (op) {
    if (op.type === 'splice') {
      ...
      if (deletion) {
        const previousSiteOp = {site: deletion.opId.site, seq: deletion.opId.seq - 1}
        this.addOperationDependency(this.deferredOperationsByDependencyId, previousSiteOp, op)
      ...
      if (insertion) {
        const previousSiteOp = {site: insertion.opId.site, seq: insertion.opId.seq - 1}
        this.addOperationDependency(this.deferredOperationsByDependencyId, previousSiteOp, op)
      ...
      if (deletion) {
        this.addOperationDependency(this.deferredOperationsByDependencyId, deletion.opId, op)
      }

      if (insertion) {
        this.addOperationDependency(this.deferredOperationsByDependencyId, insertion.opId, op)
      }
    } else {
  ...
  updateMarkersForOperations (operations) {
    ...
    for (let i = 0; i < operations.length; i++) {
      ...
      } else if (operation.type === 'splice' && operation.insert) {
        this.integrateDeferredMarkerUpdates(markerUpdates, operation.insert)
      }
    }
  ...
  textUpdatesForOperations (operations, oldUndoCounts) {
    ...
    for (let i = 0; i < operations.length; i++) {
      const operation = operations[i]
      const {type, deletion, insertion} = operation

      if (deletion) {
        const opIdString = opIdToString(deletion.opId)
        if (type !== 'undo') newOperationIds.add(opIdString)
