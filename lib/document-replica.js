...
class DocumentReplica {
  ...
  applyRemoteInsertion ({siteId, sequenceNumber, parentSiteId, parentSequenceNumber, positionInParent, text}) {
    ...
    while (insertionIndex < this.fragments.length) {
      ...
      fragmentEnd = fragmentStart + fragment.text.length
      insertionIndex++
    ...
    if (positionInParent < fragmentEnd) {
      const {prefix, suffix} = this.splitFragment(targetFragment, positionInParent - targetFragment.offset)
      const insertion = {
        siteId,
        sequenceNumber,
        parentSiteId,
      ...
      this.insertions.set(getId(insertion), [insertion])
      this.fragments.splice(insertionIndex, 1, prefix, insertion, suffix)
      return {type: 'insert', position: fragmentStart + (positionInParent - targetFragment.offset), text}
    } else {
      while (insertionIndex < this.fragments.length) {
...
// if (fragmentStart < positionInParent && positionInParent < fragmentEnd) {
//   const {prefix, suffix} = this.splitFragment(fragment, positionInParent - fragment.offset)
//   const insertion = {
//     siteId,
//     sequenceNumber,
//     parentSiteId,
