...
class DocumentReplica {
        ...
        for (let markerId in layerUpdate) {
          ...
          if (markerUpdate) {
            ...
            if (markerUpdate.range) {
              marker.range = this.getLogicalRange(markerUpdate.range)
            }
  ...
  getLogicalRange ({start, end}) {
    return {
      start: this.getLogicalPosition(start),
      end: this.getLogicalPosition(end)
    }
  }

  getLogicalPosition (position) {
    const {segment: leftDependency, start, end} = this.documentTree.findSegmentContainingPosition(position)
    const offsetInLeftDependency = traverse(leftDependency.offset, traversal(position, start))
  ...
  resolveLogicalRange ({start, end, isEmpty}, exclusive) {
    if (exclusive) {
      if (logicalPositionsEqual(start, end)) {
        const position = this.resolveLogicalPosition(start.rightDependencyId, start.offsetInRightDependency)
        return {
          start: position, end: position
        }
      } else {
        return {
          start: this.resolveLogicalPosition(start.rightDependencyId, start.offsetInRightDependency),
          end: this.resolveLogicalPosition(end.leftDependencyId, end.offsetInLeftDependency)
        }
      }
    } else {
      return {
        start: this.resolveLogicalPosition(start.leftDependencyId, start.offsetInLeftDependency),
        end: this.resolveLogicalPosition(end.rightDependencyId, end.offsetInRightDependency)
      }
    }
  }

  resolveLogicalPosition (opId, offset) {
...
function logicalPositionsEqual (a, b) {
  return (
    opIdsEqual(a.leftDependencyId, b.leftDependencyId) &&
    compare(a.offsetInLeftDependency, b.offsetInLeftDependency) === 0 &&
    opIdsEqual(a.rightDependencyId, b.rightDependencyId) &&
    compare(a.offsetInRightDependency, b.offsetInRightDependency) === 0
  )
}
