...
class DocumentReplica {
  ...
  undo () {
    ...
    if (spliceIndex != null) {
      ...
      let markerUpdates
      if (markersSnapshot) {
        const result = this.restoreMarkersSnapshot(markersSnapshot)
      ...
      return {operations, textUpdates, markerUpdates}
    } else {
      return null
    }
  }
  ...
  revertToCheckpoint (checkpointId, options) {
    ...
    const result = this.collectOperationsSinceCheckpoint(checkpointId, options && options.deleteCheckpoint)
    if (result) {
      return this.undoOrRedoOperations(result.operations)
    } else {
      return false
    }
  }
    ...
    while (i < operations.length) {
      ...
      if (this.canIntegrateOperation(operation)) {
        ...
        switch (operation.type) {
          case 'splice':
            if (operation.deletion) this.deleteRemote(operation.spliceId, operation.deletion)
            if (operation.insertion) this.insertRemote(operation.spliceId, operation.insertion)
  ...
  insertRemote (spliceId, operation) {
  ...
  deleteRemote (spliceId, deletion) {
    const {
  ...
  undoRemote ({spliceId, undoCount}, oldUndoCounts) {
  ...
  getMarkersSnapshot () {
    ...
    const layersById = this.markerLayersBySiteId.get(this.siteId)
    layersById.forEach((markersById, layerId) => {
      const snapshotMarkersById = new Map()
      markersById.forEach((marker, markerId) => {
        snapshotMarkersById.set(markerId, Object.assign({}, marker))
      })
      snapshot.set(layerId, snapshotMarkersById)
    })
    return snapshot
  }

  restoreMarkersSnapshot (snapshot) {
    const markerUpdates = {}
    const operation = {
      type: 'markers-update', siteId: this.siteId, updates: {}
    }
    const layersById = this.markerLayersBySiteId.get(this.siteId)
    ...
    snapshot.forEach((snapshotMarkersById, layerId) => {
      markerUpdates[layerId] = {}
      operation.updates[layerId] = {}

      let markersById = layersById.get(layerId)
      if (markersById) {
        markersById.forEach((marker, markerId) => {
          if (!snapshotMarkersById.has(markerId)) {
        ...
        markersById = new Map()
        layersById.set(layerId, markersById)
      }

      snapshotMarkersById.forEach((snapshotMarker, markerId) => {
        const marker = markersById.get(markerId)
        if (!marker || !markersEqual(marker, snapshotMarker)) {
          markersById.set(markerId, snapshotMarker)
          markerUpdates[layerId][markerId] = Object.assign({}, snapshotMarker)
          markerUpdates[layerId][markerId].range = this.resolveLogicalRange(snapshotMarker.range)
          operation.updates[layerId][markerId] = Object.assign({}, snapshotMarker)
        }
      })
    })

    return {operation, markerUpdates}
  }
