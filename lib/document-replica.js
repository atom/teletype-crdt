...
class DocumentReplica {
  ...
  revertToCheckpoint (checkpointId, options) {
    ...
    if (collectResult) {
      ...
      const restoreResult = this.restoreMarkersSnapshot(collectResult.markersSnapshot)
      operations.push(restoreResult.operation)
      const {markerUpdates} = restoreResult
      return {operations, textUpdates, markerUpdates}
    } else {
      return false
    }
  ...
  getMarkersSnapshot () {
    ...
    layersById.forEach((markersById, layerId) => {
      snapshot.set(layerId, new Map(markersById))
    })
    return snapshot
  }
    ...
    snapshot.forEach((snapshotMarkersById, layerId) => {
      ...
      snapshotMarkersById.forEach((snapshotMarker, markerId) => {
        ...
        if (!marker || !markersEqual(marker, snapshotMarker)) {
          ...
          operation.updates[layerId][markerId] = snapshotMarker
        }
      })
    })
