...
class DocumentReplica {
  ...
  deleteLocal ({position, extent}) {
    ...
    const preLeft = this.findLocalSegmentBoundary(position)[0]
  ...
  splitSegment (segment, offset) {
    ...
    if (prefix.deletions) prefix.deletions = new Set(prefix.deletions)
    ...
    if (suffix.deletions) suffix.deletions = new Set(suffix.deletions)
    if (suffix.deletionEnds) suffix.deletionEnds = new Set(suffix.deletionEnds)
