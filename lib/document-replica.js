...
class DocumentReplica {
  constructor (siteId) {
    ...
    const firstSegment = {opId: {site: 0, seq: 0}, offset: ZERO_POINT, pos: 0, text: '', nextSplit: null, deletions: new Set()}
    this.splitTreesByOpId.set(opIdToString(firstSegment.opId), new SplitTree(firstSegment))

    const lastSegment = {opId: {site: 0, seq: 1}, offset: ZERO_POINT, pos: 1, text: '', nextSplit: null, deletions: new Set()}
  ...
  insertLocal ({position, text}) {
    ...
    return {
      ...
      offsetInLeftDependency: traverse(left.offset, extentForText(left.text)),
  ...
  deleteLocal ({position, extent}) {
    ...
    while (true) {
      ...
      segmentRange.endOffset = traverse(segment.offset, extentForText(segment.text))
  ...
  updateUndoCount (opId, newUndoCount) {
        ...
        if (wasVisible) {
          if (!isVisible) {
            opsToApply.push({
              ...
              extent: extentForText(segment.text),
              pos: segment.pos
            })
          }
        } else {
    ...
    return opsToApply.sort((a, b) => b.pos - a.pos)
  }
  ...
  deleteRemote ({opId: deletionOpId, offsetRanges}) {
    ...
    for (let i = 0; i < offsetRanges.length; i++) {
      ...
      let segment = left
      while (true) {
        const extent = extentForText(segment.text)
  ...
  findSegmentStart (opId, offset) {
    ...
    const segmentEndOffset = traverse(segment.offset, extentForText(segment.text))
