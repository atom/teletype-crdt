...
class DocumentReplica {
  ...
  undo () {
    let stackEntry
    while ((stackEntry = this.undoStack.pop())) {
      this.redoStack.push(stackEntry)
      if (stackEntry instanceof Transaction) {
        return this.undoOrRedoOperations(stackEntry.operations)
  ...
  redo () {
    let stackEntry
    while ((stackEntry = this.redoStack.pop())) {
  ...
  createCheckpoint (props) {
    const checkpoint = new Checkpoint(this.nextCheckpointId++, props && props.isBarrier)
    this.undoStack.push(checkpoint)
    return checkpoint.id
  }

  groupChangesSinceCheckpoint (checkpointId, _, deleteCheckpoint = false) {
    const operationsSinceCheckpoint = this.collectOperationsSinceCheckpoint(checkpointId, deleteCheckpoint)
