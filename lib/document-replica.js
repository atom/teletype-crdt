...
class DocumentReplica {
  ...
  updateMarkerLayers (layerUpdatesById) {
    const operation = {
      ...
      opId: {site: this.siteId, seq: this.nextSequenceNumber++},
      siteId: this.siteId,
      updates: {}
    }
      ...
      if (layerUpdate === null) {
        ...
        for (let markerId in layerUpdate) {
          ...
          if (markerUpdate) {
            ...
            if (markerUpdate.range) {
              marker.range = {
                start: this.getRemotePosition(markerUpdate.range.start),
                end: this.getRemotePosition(markerUpdate.range.end)
              }
            }
  ...
  undoOrRedoOperations (operationsToUndo) {
    ...
    return {
      ...
      textUpdates: this.deltaForOperations(undoOperations, oldUndoCounts)
    }
  }
  ...
  integrateOperations (operations) {
    operations = operations.filter(op => {
      if (this.canIntegrateOperation(op)) {
        return true
      } else {
        this.deferOperation(op)
        return false
      }
    })

    const markerLayerUpdates = {}
    ...
    while (i < operations.length) {
      ...
      switch (operation.type) {
        ...
        case 'undo':
          ...
          break
        case 'marker-layers-update':
          this.integrateMarkerLayersUpdate(operation, markerLayerUpdates)
          break
      }
      ...
      const dependentMarkerUpdates = this.deferredMarkerUpdatesByDependencyId.get(operationIdString)
      if (dependentMarkerUpdates) {
        dependentMarkerUpdates.forEach(({siteId, layerId, markerId}) => {
          const deferredForSite = this.deferredMarkerUpdates.get(siteId)
          if (deferredForSite) {
            const deferredForLayer = deferredForSite.get(layerId)
            if (deferredForLayer) {
              const markerUpdate = deferredForLayer.get(markerId)
              if (markerUpdate && this.canTranslateRemoteRange(markerUpdate.range)) {
                this.integrateMarkerUpdate(markerLayerUpdates, siteId, layerId, markerId, markerUpdate)
              }
              deferredForLayer.delete(markerId)
              if (deferredForLayer.size === 0) deferredForSite.delete(layerId)
              if (deferredForSite.size === 0) this.deferredMarkerUpdates.delete(siteId)
            }
          }
        })
        this.deferredMarkerUpdatesByDependencyId.delete(operationIdString)
      }
    ...
    return {
      textUpdates: this.deltaForOperations(operations, oldUndoCounts),
      markerLayerUpdates
    }
  }
  ...
  integrateMarkerLayersUpdate ({siteId, updates}, reportedUpdates) {
    ...
    if (!reportedUpdates[siteId]) reportedUpdates[siteId] = {}
    ...
    for (let layerId in updates) {
      ...
      if (markerUpdates) {
        ...
        if (!reportedUpdates[siteId][layerId]) reportedUpdates[siteId][layerId] = {}

        for (let markerId in markerUpdates) {
          markerId = parseInt(markerId)
          ...
          if (markerUpdate) {
            if (markerUpdate.range && !this.canTranslateRemoteRange(markerUpdate.range)) {
              ...
              this.addOperationDependency(this.deferredMarkerUpdatesByDependencyId, range.start.leftDependencyId, deferredMarkerUpdate)
              this.addOperationDependency(this.deferredMarkerUpdatesByDependencyId, range.start.rightDependencyId, deferredMarkerUpdate)
              this.addOperationDependency(this.deferredMarkerUpdatesByDependencyId, range.end.leftDependencyId, deferredMarkerUpdate)
              this.addOperationDependency(this.deferredMarkerUpdatesByDependencyId, range.end.rightDependencyId, deferredMarkerUpdate)
              ...
              this.integrateMarkerUpdate(reportedUpdates, siteId, layerId, markerId, markerUpdate)
            }
          } else {
            if (layer.has(markerId)) {
              layer.delete(markerId)
              reportedUpdates[siteId][layerId][markerId] = null
            }
          }
        }
      } else {
        if (layer) {
          reportedUpdates[siteId][layerId] = null
        }
      }
    }
  }

  integrateMarkerUpdate (reportedUpdates, siteId, layerId, markerId, markerUpdate) {
    const layer = this.markerLayersBySiteId.get(siteId).get(layerId)
    let marker = layer.get(markerId)
    if (!marker) {
      marker = {}
      layer.set(markerId, marker)
    ...
    if (!reportedUpdates[siteId]) reportedUpdates[siteId] = {}
    if (!reportedUpdates[siteId][layerId]) reportedUpdates[siteId][layerId] = {}
    reportedUpdates[siteId][layerId][markerId] = Object.assign({}, marker)
    reportedUpdates[siteId][layerId][markerId].range = {
      start: this.getLocalPositionSync(marker.range.start),
      end: this.getLocalPositionSync(marker.range.end)
    }
  }
  ...
  canTranslateRemoteRange (remoteRange) {
    return (
      this.canTranslateRemotePosition(remoteRange.start) &&
      this.canTranslateRemotePosition(remoteRange.end)
    )
  }
