...
class DocumentReplica {
  ...
  setTextInRange (start, extent, text) {
    ...
    this.undoStack.push(remoteOperations)
    return remoteOperations
  }

  undo () {
    const remoteOperations = this.undoStack.pop()
    const operationsToApply = []
    const operationsToSend = []
    for (var i = 0; i < remoteOperations.length; i++) {
      const remoteOperation = remoteOperations[i]
    ...
    this.redoStack.push(remoteOperations)
    return {opsToApply: operationsToApply, opsToSend: operationsToSend}
  }

  redo () {
    const remoteOperations = this.redoStack.pop()
    const operationsToApply = []
    const operationsToSend = []
    for (var i = 0; i < remoteOperations.length; i++) {
      const remoteOperation = remoteOperations[i]
