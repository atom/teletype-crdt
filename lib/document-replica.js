...
class DocumentReplica {
  ...
  canApplyRemote (operation) {
    return this.regionsByInsertionId.has(getParentInsertionId(operation))
  }
  ...
  applyRemoteInsertion ({siteId, sequenceNumber, parentSiteId, parentSequenceNumber, positionInParent, text}) {
    let targetRegion
    const regions = this.regionsByInsertionId.get(getParentInsertionId({parentSiteId, parentSequenceNumber}))
