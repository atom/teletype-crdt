...
class DocumentReplica {
  ...
  setTextInRange (start, extent, text) {
    const remoteOperations = []
    if (compare(extent, ZERO_POINT) > 0) {
      remoteOperations.push(this.delete(start, extent))
    }
  ...
  deltaForOperations (operations) {
    ...
    const positionsBySegment = new Map()
      ...
      if (insertionSplitTree) {
        ...
        for (let i = 0; i < splitSegments.length; i++) {
          ...
          if (this.isSegmentVisible(segment)) {
            ...
            positionsBySegment.set(segment, this.documentTree.getSegmentPosition(segment))
          }
        }
      } else {
        const {
    ...
    const orderedSegments = Array.from(segments).sort((s1, s2) => compare(positionsBySegment.get(s1), positionsBySegment.get(s2)))
    const changes = []
    let lastChange
    for (let i = 0; i < orderedSegments.length; i++) {
      const segment = orderedSegments[i]
      const opIdString = opIdToString(segment.opId)
      const segmentNewStart = positionsBySegment.get(segment)
      let segmentOldStart
      if (lastChange) {
        segmentOldStart = traverse(lastChange.oldEnd, traversal(segmentNewStart, lastChange.newEnd))
      } else {
        segmentOldStart = segmentNewStart
      }
      ...
      } else if (!insertions.has(opIdString)) {
