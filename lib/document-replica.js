...
class DocumentReplica {
  ...
  setTextInRange (start, end, text) {
    ...
    if (compare(end, start) > 0) {
      remoteOperations.push(this.delete(start, end))
    }
    if (text && text.length > 0) {
      remoteOperations.push(this.insert(start, text))
    }

    this.undoStack.push(new Transaction(remoteOperations))
  ...
  undo () {
    ...
    for (var i = 0; i < operationsToUndo.length; i++) {
      const remoteOperation = operationsToUndo[i]
      const {opsToApply, opToSend} = this.undoOrRedoOperation(remoteOperation.opId)
  ...
  redo () {
    ...
    for (var i = 0; i < operationsToRedo.length; i++) {
      const remoteOperation = operationsToRedo[i]
      const {opsToApply, opToSend} = this.undoOrRedoOperation(remoteOperation.opId)
      operationsToApply.push(...opsToApply)
      operationsToSend.push(opToSend)
    }
  ...
  applyRemoteOperation (operation) {
    ...
    if (this.canApplyRemoteOperation(operation)) {
      ...
      while (remoteOpsToApply.length > 0) {
        ...
        if (dependentOps) {
          dependentOps.forEach((dependentOp) => {
            if (this.canApplyRemoteOperation(dependentOp)) {
        ...
        const dependentResolutions = this.deferredResolutionsByDependencyId.get(remoteOpToApplyIdString)
        if (dependentResolutions) {
          dependentResolutions.forEach((resolve) => resolve())
          this.deferredResolutionsByDependencyId.delete(remoteOpToApplyIdString)
        }
      }
    } else {
  ...
  deferRemote (op) {
    if (op.type === 'insert') {
      this.addRemoteOpDependency(this.deferredOperationsByDependencyId, {site: op.opId.site, seq: op.opId.seq - 1}, op)
      this.addRemoteOpDependency(this.deferredOperationsByDependencyId, op.leftDependencyId, op)
      this.addRemoteOpDependency(this.deferredOperationsByDependencyId, op.rightDependencyId, op)
      ...
      for (const site in op.maxSeqsBySite) {
        ...
        this.addRemoteOpDependency(this.deferredOperationsByDependencyId, {site, seq}, op)
      }
    } else if (op.type === 'undo') {
      this.addRemoteOpDependency(this.deferredOperationsByDependencyId, op.opId, op)
    } else {
      throw new Error('Unknown operation type: ' + op.type)
    }
  }

  addRemoteOpDependency (map, dependencyId, op) {
  ...
  deltaForOperations (operations) {
    ...
    for (let i = 0; i < operations.length; i++) {
      ...
      if (insertionSplitTree) {
        insertions.add(opIdString)
        const splitSegments = insertionSplitTree.getSegments()
        ...
        while (true) {
          const maxSeq = maxSeqsBySite[segment.opId.site] || 0
          if (segment.opId.seq <= maxSeq && !this.isSegmentVisible(segment)) {
            segmentStartPositions.set(segment, this.documentTree.getSegmentPosition(segment))
            segmentIndices.set(segment, this.documentTree.getSegmentIndex(segment))
          }
    ...
    const orderedSegments = Array.from(segmentIndices.keys()).sort((s1, s2) => segmentIndices.get(s1) - segmentIndices.get(s2))
    const changes = []
    let lastChange

    for (let i = 0; i < orderedSegments.length; i++) {
      const segment = orderedSegments[i]
      const segmentIsVisibleWithOperations = this.isSegmentVisible(segment)
      const segmentIsVisibleWithoutOperations = (
        !insertions.has(opIdToString(segment.opId)) &&
        this.isSegmentVisible(segment, null, null, deletions)
      )

      if (segmentIsVisibleWithOperations !== segmentIsVisibleWithoutOperations) {
        ...
        if (segmentIsVisibleWithOperations) {
  ...
  getLocalPosition (remotePosition) {
    if (this.canTranslateRemotePosition(remotePosition)) {
      ...
      return new Promise((resolve) => {
        ...
        this.addRemoteOpDependency(this.deferredResolutionsByDependencyId, remotePosition.leftDependencyId, resolveWithLocalPosition)
        this.addRemoteOpDependency(this.deferredResolutionsByDependencyId, remotePosition.rightDependencyId, resolveWithLocalPosition)
      })
    }
  }
  ...
  isSegmentVisible (segment, opId, newUndoCount, deletionsToIgnore) {
    ...
    return (undoCount & 1) === 0 && !this.isSegmentDeleted(segment, opId, newUndoCount, deletionsToIgnore)
  }

  isSegmentDeleted (segment, opId, undoCount, deletionsToIgnore) {
    const opIdString = opId ? opIdToString(opId) : null
    for (const deletionOpIdString of segment.deletions) {
      if (deletionsToIgnore && deletionsToIgnore.has(deletionOpIdString)) {
        continue
      }
