      ...
      if (isIncludedInVersion(prevVersion, segment.spliceId)) {
        ...
        if (isInsertionUndone) {
          if (!wasInsertionUndone && !wasInsertionDeleted) {
            ...
            changeset.push(start, ZERO_POINT, '', segment.extent, segment.text)
          }
        } else {
          if (wasInsertionUndone) {
            if (!activeDeletion) {
              ...
              const start = this.documentTree.getSegmentPosition(segment)
              changeset.push(start, segment.extent, segment.text, ZERO_POINT, '')
            }
          } else {
...
class Changeset {
  ...
  push(newStart, newExtent, newText, oldExtent, oldText) {
