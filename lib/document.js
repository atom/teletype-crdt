...
class Document {
  ...
  */
  getChangesSinceVersion(prevVersion) {
    ...
    for (const segment of this.documentTree.getSegments()) {
      if (isIncludedInVersion(prevVersion, segment.spliceId)) {
        ...
        if (isInsertionUndone) {
          if (!wasInsertionUndone && !wasInsertionDeleted) {
            ...
            const start = this.documentTree.getSegmentPosition(segment)
            const {site} = curInsertionUndo.id
            changeset.push(start, ZERO_POINT, '', segment.extent, segment.text, site)
          }
        } else {
          if (wasInsertionUndone) {
            if (!activeDeletion) {
              ...
              const start = this.documentTree.getSegmentPosition(segment)
              const {site} = curInsertionUndo.id
              changeset.push(start, segment.extent, segment.text, ZERO_POINT, '', site)
            }
          } else {
...
class Changeset {
  ...
  push(newStart, newExtent, newText, oldExtent, oldText, authorSiteId = null) {
    const prevChange = this.changes[this.changes.length - 1]

    if (prevChange && compare(newStart, prevChange.newEnd) === 0) {
      prevChange.oldEnd = traverse(prevChange.oldEnd, oldExtent)
      prevChange.oldText += oldText
      prevChange.newEnd = traverse(prevChange.newEnd, newExtent)
      prevChange.newText += newText
    } else {
      let oldStart
      if (prevChange) {
        oldStart = traverse(prevChange.oldEnd, traversal(newStart, prevChange.newEnd))
      } else {
        oldStart = newStart
      }

      this.changes.push({
        newStart, newEnd: traverse(newStart, newExtent), newText,
        oldStart, oldEnd: traverse(oldStart, oldExtent), oldText
      })
    }

    if (authorSiteId != null) {
      this.changes[this.changes.length - 1].author = authorSiteId
    }
  }
