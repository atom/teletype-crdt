...
class Document {
  constructor ({siteId, text, history}) {
    ...
    this.operations = []
    this.undoOperationsBySpliceId = new Map()
  ...
  */
  ...
  /*
  Public: Returns an {Object} representing the version of this document.
  */
  getVersion() {
    return {...this.maxSeqsBySite}
  }

  /*
  Public: Computes all the changes that occurred since the specified version.

  * `prevVersion` Version {Object} previously obtained via `Document.prototype.getVersion()`

  Returns an array of change {Object}s of shape:
    ...
    * `newEnd` Point {Object}
    * `newText` {String}
    * `author` {Number} representing the site id that caused this change
  */
  getChangesSinceVersion (prevVersion) {
    const curVersion = this.maxSeqsBySite

    const changeset = new Changeset()

    let segmentStart = ZERO_POINT
    let segment = this.documentTree.firstSegment
    while (segment != null) {
      if (isIncludedInVersion(prevVersion, segment.spliceId)) {
        const prevInsertionUndo = this.undoForSpliceAtVersion(segment.spliceId, prevVersion)
        const wasInsertionUndone = prevInsertionUndo ? (prevInsertionUndo.undoCount & 1) === 1 : false

        const curInsertionUndo = this.undoForSpliceAtVersion(segment.spliceId, curVersion)
        const isInsertionUndone = curInsertionUndo ? (curInsertionUndo.undoCount & 1) === 1 : false

        let wasInsertionDeleted = false
        let activeDeletion
        let deletionUndo
        for (let deletionId of segment.deletions) {
          deletionId = spliceIdFromString(deletionId)

          const prevUndo = this.undoForSpliceAtVersion(deletionId, prevVersion)
          const wasUndone = prevUndo ? (prevUndo.undoCount & 1) === 1 : false

          const curUndo = this.undoForSpliceAtVersion(deletionId, curVersion)
          const isUndone = curUndo ? (curUndo.undoCount & 1) === 1 : false

          if (isIncludedInVersion(prevVersion, deletionId) && !wasUndone) {
            wasInsertionDeleted = true
            if (isUndone) {
              deletionUndo = curUndo
            }
          }

          if (!isUndone) {
            activeDeletion = deletionId
            if (wasUndone) {
              deletionUndo = curUndo
            }
          }
        ...
        if (isInsertionUndone) {
          if (!wasInsertionUndone && !wasInsertionDeleted) {
            // Report deletion (due to undo)
            const {site} = curInsertionUndo.id
            changeset.push(segmentStart, ZERO_POINT, '', segment.extent, segment.text, site)
          }
        } else {
          if (wasInsertionUndone) {
            if (!activeDeletion) {
              // Report insertion (due to redo)
              const {site} = curInsertionUndo.id
              changeset.push(segmentStart, segment.extent, segment.text, ZERO_POINT, '', site)
            }
          } else {
            if (wasInsertionDeleted && !activeDeletion) {
              // Report insertion (due to undoing a deletion)
              const {site} = deletionUndo.id
              changeset.push(segmentStart, segment.extent, segment.text, ZERO_POINT, '', site)
            } else if (!wasInsertionDeleted && activeDeletion) {
              // Report deletion
              const {site} = isIncludedInVersion(prevVersion, activeDeletion) ? deletionUndo.id : activeDeletion
              changeset.push(segmentStart, ZERO_POINT, '', segment.extent, segment.text, site)
            }
          }
        }
      } else if (this.isSegmentVisible(segment)) {
        // Report insertion
        const {site} = segment.spliceId
        changeset.push(segmentStart, segment.extent, segment.text, ZERO_POINT, '', site)
      }

      if (this.isSegmentVisible(segment)) {
        segmentStart = traverse(segmentStart, segment.extent)
      }
  ...
  */
  ...
  undoOrRedoOperations (operationsToUndo) {
    ...
    for (var i = 0; i < operationsToUndo.length; i++) {
      ...
      const operation = {type: 'undo', id, spliceId, undoCount: newUndoCount}
      undoOperations.push(operation)
      this.pushOperation(operation)
      this.updateMaxSeqsBySite(id)
    }
  ...
  undoForSpliceAtVersion (spliceId, version) {
    const undoOperations = this.undoOperationsBySpliceId.get(spliceIdToString(spliceId))
    if (undoOperations) {
      let maxUndo
      for (const operation of undoOperations) {
        if (isIncludedInVersion(version, operation.id)) {
          if (!maxUndo || operation.undoCount > maxUndo.undoCount) {
            maxUndo = operation
          }
        }
      }
  ...
  canIntegrateOperation (op) {
    switch (op.type) {
      ...
      case 'undo': {
        if ((this.maxSeqsBySite[op.id.site] || 0) !== op.id.seq - 1) {
          return false
        }
  ...
  */
  integrateOperations (operations) {
    ...
    while (i < operations.length) {
      ...
      if (operation.type !== 'markers-update') this.pushOperation(operation)
      ...
      if (this.canIntegrateOperation(operation)) {
        ...
        switch (operation.type) {
          ...
          case 'undo':
            ...
            this.updateMaxSeqsBySite(operation.id)
            break
        }

        if (operation.id) {
          this.collectDeferredOperations(operation.id, operations)
        }
        if (operation.spliceId) {
          this.collectDeferredOperations(operation.spliceId, operations)
        }
      } else {
  ...
  collectDeferredOperations (id, operations) {
    const key = spliceIdToString(id)
    const dependentOps = this.deferredOperationsByDependencyId.get(key)
    if (dependentOps) {
      ...
      this.deferredOperationsByDependencyId.delete(key)
    }
  }
  ...
  deferOperation (op) {
    if (op.type === 'splice') {
      ...
      this.addOperationDependency(this.deferredOperationsByDependencyId, {site: op.id.site, seq: op.id.seq - 1}, op)
  ...
  pushOperation (operation) {
    ...
    if (operation.type === 'undo') {
      const key = spliceIdToString(operation.spliceId)
      let undoOps = this.undoOperationsBySpliceId.get(key)
      if (!undoOps) {
        undoOps = []
        this.undoOperationsBySpliceId.set(key, undoOps)
      }
  ...
  addOperationDependency (map, dependencyId, op) {
    ...
    if (dependencyId.seq > 0 || !this.hasAppliedSplice(dependencyId)) {
  ...
  updateUndoCount (spliceId, newUndoCount, oldUndoCounts) {
    ...
    if (!oldUndoCounts.has(spliceIdString)) {
    ...
    const countsBeforeUndo = new Map([[spliceIdString, previousUndoCount]])
    segmentsToUpdate.forEach((segment) => {
      const wasVisible = this.isSegmentVisible(segment, countsBeforeUndo)
  ...
  computeChangesForSegments (segmentIndices, segmentStartPositions, oldUndoCounts, newOperations) {
    ...
    for (let i = 0; i < orderedSegments.length; i++) {
      ...
      if (visibleBefore !== visibleAfter) {
        const segmentStart = segmentStartPositions.get(segment)
        if (visibleBefore) {
          changeset.push(segmentStart, ZERO_POINT, '', segment.extent, segment.text)
        } else {
          changeset.push(segmentStart, segment.extent, segment.text, ZERO_POINT, '')
        }
      }
    }
...
function spliceIdFromString (spliceId) {
  const [site, seq] = spliceId.split('.')
  return {site: Number(site), seq: Number(seq)}
}
...
function isIncludedInVersion (version, spliceId) {
  const latestOperationInVersion = version[spliceId.site] || 0
  return latestOperationInVersion >= spliceId.seq
}
...
class Changeset {
  ...
  push (newStart, newExtent, newText, oldExtent, oldText, authorSiteId) {
    const prev = this.changes[this.changes.length - 1]
    let oldStart
    if (prev) {
      oldStart = traverse(prev.oldEnd, traversal(newStart, prev.newEnd))
    } else {
      oldStart = newStart
    }
    ...
    const change = {
      newStart,
      newEnd: traverse(newStart, newExtent),
      newText,
      oldStart,
      oldEnd: traverse(oldStart, oldExtent),
      oldText
    }
    if (authorSiteId != null) {
      change.author = authorSiteId
    }
  ...
  getChanges () {
    ...
    while (this.changes.length > 0) {
      i = Math.min(i, this.changes.length - 1)
      const prev = this.changes[i - 1]
      const cur = this.changes[i]

      if (isEmptyChange(cur)) {
        this.changes.splice(i, 1)
      } else if (!prev) {
        break
      } else if (compare(cur.newStart, prev.newEnd) === 0) {
        const curNewExtent = traversal(cur.newEnd, cur.newStart)
        const curOldExtent = traversal(cur.oldEnd, cur.oldStart)

        const newHunk = prev.newText + cur.newText
        const oldHunk = prev.oldText + cur.oldText
        const prefix = getPrefix(newHunk, oldHunk)

        if (cur.author === prev.author) {
          prev.newStart = traverse(prev.newStart, prefix.extent)
          prev.newEnd = maxPoint(prev.newStart, traverse(prev.newEnd, curNewExtent))
          prev.newText = newHunk.substr(prefix.length)
          prev.oldStart = traverse(prev.oldStart, prefix.extent)
          prev.oldEnd = maxPoint(prev.oldStart, traverse(prev.oldEnd, curOldExtent))
          prev.oldText = oldHunk.substr(prefix.length)

          this.changes.splice(i, 1)
        } else if (prefix.length !== 0) {
          prev.newStart = traverse(prev.newStart, prefix.extent)
          prev.newEnd = maxPoint(prev.newEnd, prev.newStart)
          prev.newText = prev.newText.substr(prefix.length)
          prev.oldStart = traverse(prev.oldStart, prefix.extent)
          prev.oldEnd = maxPoint(prev.oldEnd, prev.oldStart)
          prev.oldText = prev.oldText.substr(prefix.length)

          const prefixInCurNewText = traversal(maxPoint(cur.newStart, prev.newEnd), cur.newStart)
          const prefixInCurOldText = traversal(maxPoint(cur.oldStart, prev.oldEnd), cur.oldStart)
          cur.newStart = prev.newEnd
          cur.newEnd = traverse(cur.newStart, traversal(curNewExtent, prefixInCurNewText))
          cur.newText = newHunk.substr(prefix.length + prev.newText.length)
          cur.oldStart = prev.oldEnd
          cur.oldEnd = traverse(cur.oldStart, traversal(curOldExtent, prefixInCurOldText))
          cur.oldText = oldHunk.substr(prefix.length + prev.oldText.length)
