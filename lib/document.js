...
class Document {
  ...
  integrateInsertion (spliceId, operation) {
    ...
    if (originalRightDependency.documentLeft !== originalLeftDependency) {
      this.documentTree.rotateNodeRight(originalLeftDependency)
    }

    let successor = originalRightDependency
    let segment = originalLeftDependency.documentRight
    while (segment) {
    ...
    this.documentTree.insertBetween(this.documentTree.getPredecessor(successor), successor, newSegment)
    this.splitTreesBySpliceId.set(spliceIdToString(spliceId), new SplitTree(newSegment))
  }
