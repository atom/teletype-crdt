...
class Changeset {
  ...
  push (newStart, newExtent, newText, oldExtent, oldText, authorSiteId) {
    ...
    if (prev && compare(newStart, prev.newEnd) === 0) {
      const newHunk = prev.newText + newText
      const oldHunk = prev.oldText + oldText
      const prefix = getPrefix(newHunk, oldHunk)

      if (authorSiteId === prev.author) {
        prev.newStart = traverse(prev.newStart, prefix.extent)
        prev.newEnd = maxPoint(prev.newStart, traverse(prev.newEnd, newExtent))
        prev.newText = newHunk.substr(prefix.length)
        prev.oldStart = traverse(prev.oldStart, prefix.extent)
        prev.oldEnd = maxPoint(prev.oldStart, traverse(prev.oldEnd, oldExtent))
        ...
        const newSuffixExtent = traversal(newExtent, minPoint(prefix.extent, newExtent))
        const oldSuffixExtent = traversal(oldExtent, minPoint(prefix.extent, oldExtent))
        change = {
          newStart: prev.newEnd,
          newEnd: traverse(prev.newEnd, newSuffixExtent),
          newText: newHunk.substr(prefix.length + prev.newText.length),
          oldStart: prev.oldEnd,
          oldEnd: traverse(prev.oldEnd, oldSuffixExtent),
          oldText: oldHunk.substr(prefix.length + prev.oldText.length)
        }
      }
    } else {
      let oldStart
    ...
    if (prev && isEmptyChange(prev)) {
      this.changes.pop()
    }

    if (change && !isEmptyChange(change)) {
