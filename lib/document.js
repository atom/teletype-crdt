...
class Changeset {
  ...
  push(newStart, newExtent, newText, oldExtent, oldText, authorSiteId = null) {
    ...
    if (prev && prev.author == authorSiteId && compare(newStart, prev.newEnd) === 0) {
      prev.oldEnd = traverse(prev.oldEnd, oldExtent)
      prev.oldText += oldText
      prev.newEnd = traverse(prev.newEnd, newExtent)
