...
class Document {
  ...
  integrateInsertion (spliceId, operation) {
    ...
    let predecessor = originalLeftDependency
    let link = this.documentTree.getSuccessor(predecessor)
    while (link !== originalRightDependency) {
      if (insertionClock > link.insertionClock || (insertionClock === link.insertionClock && spliceId.site < link.spliceId.site)) {
        break
      } else {
        predecessor = link
        link = this.documentTree.getSuccessor(link)
      }
    }
    ...
    this.documentTree.insertBetween(predecessor, link, newSegment)
    this.splitTreesBySpliceId.set(spliceIdToString(spliceId), new SplitTree(newSegment))
  }
