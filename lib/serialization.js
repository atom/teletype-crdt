...
function serializeDeletion (deletion) {
  ...
  deletion_.setOffsetRangesList(deletion.offsetRanges.map(({opId, startOffset, endOffset}) => {
    const offsetRange_ = new Operation.Deletion.OffsetRange()
    offsetRange_.setOpId(serializeOpId(opId))
    offsetRange_.setStartOffset(serializePoint(startOffset))
    offsetRange_.setEndOffset(serializePoint(endOffset))
    return offsetRange_
  }))
  return deletion_
}
...
function deserializeDeletion (deletion_) {
  return {
    ...
    offsetRanges: deletion_.getOffsetRangesList().map((offsetRange_) => {
      return {
        opId: deserializeOpId(offsetRange_.getOpId()),
        startOffset: deserializePoint(offsetRange_.getStartOffset()),
        endOffset: deserializePoint(offsetRange_.getEndOffset())
      }
    })
  }
