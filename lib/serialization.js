const {Operation, OperationId, Point, RemotePosition} = require('./tachyon_pb')
...
function serializeRemotePosition (position) {
  const position_ = new RemotePosition()
  position_.setSiteId(position.siteId)
  position_.setLeftDependencyId(serializeOpId(position.leftDependencyId))
  position_.setOffsetInLeftDependency(serializePoint(position.offsetInLeftDependency))
  position_.setRightDependencyId(serializeOpId(position.rightDependencyId))
  position_.setOffsetInRightDependency(serializePoint(position.offsetInRightDependency))
  return position_
}

function serializeRemotePositionBinary (position) {
  return serializeRemotePosition(position).serializeBinary()
}
...
function deserializeOperation (op_) {
  if (op_.hasInsertion()) {
...
function deserializeRemotePosition (position_) {
  return {
    siteId: position_.getSiteId(),
    leftDependencyId: deserializeOpId(position_.getLeftDependencyId()),
    offsetInLeftDependency: deserializePoint(position_.getOffsetInLeftDependency()),
    rightDependencyId: deserializeOpId(position_.getRightDependencyId()),
    offsetInRightDependency: deserializePoint(position_.getOffsetInRightDependency())
  }
}

function deserializeRemotePositionBinary (data) {
  return deserializeRemotePosition(RemotePosition.deserializeBinary(data))
}
