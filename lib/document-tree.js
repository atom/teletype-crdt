...
const {ZERO_POINT, compare, traverse, extentForText} = require('./point-helpers')
...
class DocumentTree extends SplayTree {
  ...
  findSegmentContainingPosition (position) {
    ...
    while (segment) {
      ...
      let end = start
      if (this.isSegmentVisible(segment)) end = traverse(end, extentForText(segment.text))
  ...
  updateSubtreeExtent (node) {
    ...
    if (this.isSegmentVisible(node)) node.documentSubtreeExtent = traverse(node.documentSubtreeExtent, extentForText(node.text))
