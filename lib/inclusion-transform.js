...
function transformDeleteInsert (del, ins) {
  if (compare(del.end, ins.start) <= 0) {
    return new DeleteOperation(del.start, del.text, del.siteId)
  } else {
    const comparison = compare(del.start, ins.start)
    if (comparison > 0) {
      return new DeleteOperation(traverse(ins.end, traversal(del.start, ins.start)), del.text, del.siteId)
    } else if (comparison === 0) {
      return new DeleteOperation(del.start, ins.text + del.text, del.siteId)
    } else {
      ...
      return new DeleteOperation(del.start, text, del.siteId)
    }
  }
}
...
function transformDeleteDelete (del1, del2) {
  if (compare(del1.end, del2.start) <= 0) {
    return new DeleteOperation(del1.start, del1.text, del1.siteId)
  } else if (compare(del1.start, del2.end) >= 0) {
    return new DeleteOperation(traverse(del2.start, traversal(del1.start, del2.end)), del1.text, del1.siteId)
    ...
    return new DeleteOperation(del1.start, del1.text.slice(0, prefixIndex), del1.siteId)
    ...
    return new DeleteOperation(del2.start, del1.text.slice(suffixIndex), del1.siteId)
