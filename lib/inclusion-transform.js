...
function transformInsertInsert (ins1, ins2) {
  ...
  if (positionComparison < 0 || (positionComparison === 0 && ins1.siteId > ins2.siteId)) {
    return {type: 'insert', start: ins1.start, text: ins1.text, siteId: ins1.siteId, debugId: ins1.debugId + "'"}
  } else {
    return {type: 'insert', start: traverse(getEnd(ins2), traversal(ins1.start, ins2.start)), text: ins1.text, siteId: ins1.siteId, debugId: ins1.debugId + "'"}
  }
}

function transformInsertDelete (ins, del) {
  if (compare(ins.start, del.start) < 0) {
    return {type: 'insert', start: ins.start, text: ins.text, siteId: ins.siteId, debugId: ins.debugId + "'"}
  } else if (compare(ins.start, getEnd(del)) >= 0) {
    return {type: 'insert', start: traverse(del.start, traversal(ins.start, getEnd(del))), text: ins.text, siteId: ins.siteId, debugId: ins.debugId + "'"}
  } else {
    return {type: 'null', siteId: ins.siteId, debugId: ins.debugId + "'"}
  }
}

function transformDeleteInsert (del, ins) {
  if (compare(getEnd(del), ins.start) <= 0) {
    return {type: 'delete', start: del.start, text: del.text, siteId: del.siteId, debugId: del.debugId + "'"}
  } else {
    const comparison = compare(del.start, ins.start)
    if (comparison > 0) {
      return {type: 'delete', start: traverse(getEnd(ins), traversal(del.start, ins.start)), text: del.text, siteId: del.siteId, debugId: del.debugId + "'"}
    } else if (comparison === 0) {
      return {type: 'delete', start: del.start, text: ins.text + del.text, siteId: del.siteId, debugId: del.debugId + "'"}
    } else {
      ...
      return {type: 'delete', start: del.start, text, siteId: del.siteId, debugId: del.debugId + "'"}
    }
  }
}
...
function transformDeleteDelete (del1, del2) {
  if (compare(getEnd(del1), del2.start) <= 0) {
    return {type: 'delete', start: del1.start, text: del1.text, siteId: del1.siteId, debugId: del1.debugId + "'"}
  } else if (compare(del1.start, getEnd(del2)) >= 0) {
    return {type: 'delete', start: traverse(del2.start, traversal(del1.start, getEnd(del2))), text: del1.text, siteId: del1.siteId, debugId: del1.debugId + "'"}
    ...
    return {type: 'delete', start: del1.start, text: del1.text.slice(0, prefixIndex), siteId: del1.siteId, debugId: del1.debugId + "'"}
    ...
    return {type: 'delete', start: del2.start, text: del1.text.slice(suffixIndex), siteId: del1.siteId, debugId: del1.debugId + "'"}
    ...
    return {type: 'delete', start: del1.start, text, siteId: del1.siteId, debugId: del1.debugId + "'"}
  } else {
    return {type: 'null', siteId: del1.siteId, debugId: del1.debugId + "'"}
  }
}
