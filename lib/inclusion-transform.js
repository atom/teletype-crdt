...
function transformInsertInsert (ins1, ins2) {
  ...
  if (positionComparison < 0 || (positionComparison === 0 && ins1.siteId > ins2.siteId)) {
    return new Operation('insert', ins1.start, ins1.text, ins1.siteId, ins1.isInverse)
  } else {
    return new Operation('insert', traverse(ins2.end, traversal(ins1.start, ins2.start)), ins1.text, ins1.siteId, ins1.isInverse)
  }
}

function transformInsertDelete (ins, del) {
  if (compare(ins.start, del.start) < 0) {
    return new Operation('insert', ins.start, ins.text, ins.siteId, ins.isInverse)
  } else if (compare(ins.start, del.end) >= 0) {
    return new Operation('insert', traverse(del.start, traversal(ins.start, del.end)), ins.text, ins.siteId, ins.isInverse)
  } else {
    return new NullOperation(ins.siteId, ins.isInverse)
  }
}

function transformDeleteInsert (del, ins) {
  if (compare(del.end, ins.start) <= 0) {
    return new Operation('delete', del.start, del.text, del.siteId, del.isInverse)
  } else {
    const comparison = compare(del.start, ins.start)
    if (comparison > 0) {
      return new Operation('delete', traverse(ins.end, traversal(del.start, ins.start)), del.text, del.siteId, del.isInverse)
    } else if (comparison === 0) {
      return new Operation('delete', del.start, ins.text + del.text, del.siteId, del.isInverse)
    } else {
      ...
      return new Operation('delete', del.start, text, del.siteId, del.isInverse)
    }
  }
}
...
function transformDeleteDelete (del1, del2) {
  if (compare(del1.end, del2.start) <= 0) {
    return new Operation('delete', del1.start, del1.text, del1.siteId, del1.isInverse)
  } else if (compare(del1.start, del2.end) >= 0) {
    return new Operation('delete', traverse(del2.start, traversal(del1.start, del2.end)), del1.text, del1.siteId, del1.isInverse)
    ...
    return new Operation('delete', del1.start, del1.text.slice(0, prefixIndex), del1.siteId, del1.isInverse)
    ...
    return new Operation('delete', del2.start, del1.text.slice(suffixIndex), del1.siteId, del1.isInverse)
    ...
    return new Operation('delete', del1.start, text, del1.siteId, del1.isInverse)
  } else {
    return new NullOperation(del1.siteId, del1.isInverse)
  }
}
