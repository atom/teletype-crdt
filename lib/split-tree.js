...
const {ZERO_POINT, compare, traverse, extentForText, characterIndexForPosition} = require('./point-helpers')
...
class SplitTree extends SplayTree {
  constructor (segment) {
    ...
    this.startSegment.splitSubtreeExtent = extentForText(this.startSegment.text)
  ...
  updateSubtreeExtent (node) {
    ...
    node.splitSubtreeExtent = traverse(node.splitSubtreeExtent, extentForText(node.text))
  ...
  findSegmentContainingOffset (offset) {
    ...
    while (segment) {
      ...
      const end = traverse(start, extentForText(segment.text))
