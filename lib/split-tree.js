...
class SplitTree extends SplayTree {
  ...
  splitSegment (segment, offset) {
    ...
    assert(compare(extentForText(suffix.text), suffix.extent) === 0, 'Suffix extent should match the actual extent of suffix text')
    ...
    assert(compare(extentForText(segment.text), segment.extent) === 0, 'Prefix extent should match the actual extent of prefix text')
